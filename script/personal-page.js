...
async function fetchYourCourses() {
  try {
    const token = localStorage.getItem("token");
    const response = await fetch("http://localhost:8081/course/getcoursebyuser", {
      headers: {
...
function displayYourCourses(courses) {
  const courseList = document.getElementById("your_course");
    ...
    const courseCard = `
          <div class="single_course col-lg-4">
                  ...
                  <div class="course_content">
                    <span class="price">${course.price.toLocaleString(
                      "vi-VN"
                    )} VNĐ</span>
...
async function fetchFamilierCourses() {
  try {
    const token = localStorage.getItem("token");
    const response = await fetch("http://localhost:8081/course/recommend/lstm",{
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
